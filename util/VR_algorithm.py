...
class ZMQ_VR_agent(multi_VR_agent_self):
    ...
    def combine(self, ite, style='Difffusion'):
        ...
        # weights' length is 1 more than socket list. This is because weights[0] is the self_weight
        tmp = self.weights[0]*self.phi if style == 'Diffusion' else self.weights[0]*self.cost_model.w/(self.neighbor + 1)
        for nb in range(self.neighbor):
